...
public interface StarControllerDocs {
	...
	public ApiResponse<String> addStarProduct(CustomUserDetails user, Long productId);
	
	@Operation(summary = "관심 매물 해제")
	public ApiResponse<String> removeStarProduct(CustomUserDetails user, Long productId);
	
	@Operation(summary = "내 관심 매물 목록 조회")
	public ApiResponse<List<ProductDto>> getMyStarProducts(CustomUserDetails user);
	
	// === 관심 동네 ===
	@Operation(summary = "관심 동네 등록")
	public ApiResponse<String> addStarDong(CustomUserDetails user, StarDongDto request);
	
	@Operation(summary = "관심 동네 해제")
	public ApiResponse<String> removeStarDong(CustomUserDetails user, StarDongDto request);
	
	@Operation(summary = "내 관심 동네 목록 조회")
	public ApiResponse<List<StarDongDto>> getMyStarDongs(CustomUserDetails user);
}
