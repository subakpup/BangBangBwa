      ...
      <div class="p-6 overflow-y-auto custom-scrollbar space-y-8 flex-1">
        ...
        <section class="bg-gray-50 p-5 rounded-xl border border-gray-100">
          ...
          <div v-if="form.tradeType === 'RENT'" class="space-y-4">
            <div>
              ...
              <div class="flex items-center gap-2">
                <input v-model="form.deposit.min" type="number" class="w-full p-2 border rounded-md text-sm"
                  placeholder="최소">
                <span>~</span>
                <input v-model="form.deposit.max" type="number" class="w-full p-2 border rounded-md text-sm"
                  placeholder="최대">
              </div>
            </div>
            <div>
              ...
              <div class="flex items-center gap-2">
                <input v-model="form.monthlyRent.min" type="number" class="w-full p-2 border rounded-md text-sm"
                  placeholder="최소">
                <span>~</span>
                <input v-model="form.monthlyRent.max" type="number" class="w-full p-2 border rounded-md text-sm"
                  placeholder="최대">
              </div>
            </div>
          </div>
          ...
          <div v-else>
            ...
            <div class="flex items-center gap-2">
              <input v-model="form.dealAmount.min" type="number" class="w-full p-2 border rounded-md text-sm"
                placeholder="최소">
              <span>~</span>
              <input v-model="form.dealAmount.max" type="number" class="w-full p-2 border rounded-md text-sm"
                placeholder="최대">
            </div>
          </div>
        </section>
...
const handleSearch = async () => {
  ...
  try {
    const payload = {
      ...
      deposit: (form.tradeType === 'RENT' || form.tradeType === 'LEASE')
        ? { min: form.deposit.min, max: form.deposit.max }
        : null,

      monthlyRent: (form.tradeType === 'RENT')
        ? { min: form.monthlyRent.min, max: form.monthlyRent.max }
        : null,

      dealAmount: (form.tradeType === 'SALE')
        ? { min: form.dealAmount.min, max: form.dealAmount.max }
        : null
    };
