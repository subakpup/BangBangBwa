...
//	private static final String DISCORD_WEBHOOK_URL = "https://discord.com/api/webhooks/1444633595275645071/5bm4E8wHhtwWNJOJXenct68G3JTfKQc0oOzuqrW0rse6W8Ia7i9_Rilrdai46jXvm5i0";
	...
	public PaymentResponseDto requestPreAuth(PaymentAuthDto request) {
		...
		if(request.getAccountToken() == null || request.getAccountToken().isEmpty()) {
			return PaymentResponseDto.fail("유효하지 않은 계좌 정보입니다.");
		}
	...
	public PaymentResponseDto capturePayment(PaymentCaptureDto request) {
		...
		String userBank = payment.getAccountToken();
		
		// 디스코드(가상 계좌) 연동 시도
		try {
			boolean isSent = sendNotification(payment, request.getCaptureAmount(), userBank);
			
			if(!isSent) {
				log.warn("연동 실패, 결제 거절");
				return PaymentResponseDto.fail("잔액이 부족합니다.");
			}
		} catch(Exception e) {
			log.error("계좌 연동 오류: {}", e);
			return PaymentResponseDto.fail("PG 시스템 오류");
		}
	...
	// 디스코드 알림 전송(계좌 출금)
	private boolean sendNotification(Payment payment, Long captureAmount, String webhookUrl) {
		try {
			long refundAmount = payment.getOriginalAmount() - captureAmount; // 환불 금액
	
			StringBuilder sb = new StringBuilder();
			sb.append("💳 **[결제 승인 알림]**\n");
			sb.append("> **거래유형:** 보증금 정산 (수수료 결제)\n");
			sb.append("> **주문번호:** `").append(payment.getOrderId()).append("`\n");
			sb.append("> **결제금액:** `").append(String.format("%,d", payment.getFinalAmount())).append("원`\n");
			sb.append("> **반환금액:** `").append(String.format("%,d", refundAmount)).append("원`\n");
			sb.append("> **결과:** 정상 승인 완료 ✅");
	
			Map<String, String> bodyMap = new HashMap<>();
			bodyMap.put("content", sb.toString());
			...
			log.error("디스코드 전송 중 에러 발생: {}", e.getMessage());
			return false;
		}
	}
}
